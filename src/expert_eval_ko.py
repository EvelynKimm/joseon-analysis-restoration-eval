...
def render_final_page():
    ...
    st.markdown(
        """
        참여해 주셔서 감사합니다.  
        평가 전반에 대한 총평이나 인상, 개선 의견을 자유롭게 남겨주세요.
        """
    )
...
def main():
    st.set_page_config(page_title="사료 복원 평가", layout="wide")
    ...
    if not st.session_state["intro_done"]:
        ...
        with center:
            st.title("사료 복원 전문가 평가")
            st.markdown(
                """
#### 1. 평가의 개요 및 목적

이 평가는 실제로 훼손된 『조선왕조실록』, 『승정원일기』 원문을 여러 모델이 복원한 문장으로 재구성했을 때, 전문가의 시각에서 **얼마나 역사적으로 타당하고 자연스러운 복원인가**를 정성적으로 평가하기 위한 것입니다.

이 평가는 **어떤 모델이 조선 시대 문서 복원에 더 적합한지를 파악하고, 향후 고문서 복원 시스템이 실제 복원 전문가가 활용하는 도구로 사용될 때의 개선 방향을 설정하는 것**을 최종 목표로 합니다.
...
Q1, Q2는 이러한 기준을 바탕으로, 개별 문장 수준과 모델 전체 수준에서 각각 복원 품질을 평가하기 위한 문항입니다.
...
- 제시된 각 보기는, 동일한 훼손 문장을 서로 다른 모델이 복원한 결과입니다.  
...
##### 각 문제별 코멘트
  ...
  - 특히 좋다고 느끼신 후보, 특히 문제가 있다고 판단하신 후보에 대한 의견 등  
...
해당 기간 내에 모든 문항(Q1, Q2)과 최종 코멘트 작성을 완료해 주시기를 부탁드립니다.
    ...
    st.markdown("- 각 보기는 서로 다른 모델이 생성한 동일 훼손 문장에 대한 복원 결과입니다.")
    st.markdown("- 위의 판단 기준에 따라 정답이라고 생각되는 보기들을 모두 선택해 주세요.")
    st.markdown('- 어느 후보도 정답으로 보기 어렵다면, "정답 없음"을 선택해 주세요.')
    ...
    st.markdown("---")
    st.subheader("문항 2")
    st.markdown("##### 전반적으로 가장 좋은 시스템 1개를 선택하세요.")
    st.markdown("- 후보 전체를 종합했을 때, 가장 합리적이고 자연스러운 후보를 제시하는 시스템을 선택해 주세요.")
